...
The version field must be bumped every time the extension is uploaded to the tenant.
Otherwise an error will be thrown. The tenant does not accept the same version twice.
...
available in the older versions of Dynatrace tenant or extension execution controller.
...
* ``local:`` means the extension can run on OneAgent. When you deploy the extension,
  the tenant will automatically read this value and will understand that an active
...
.. admonition:: Live schema editor
   ...
   ``https://<tenant_url>/ui/docs/settings-playground`` page on your tenant.
...
is created, then the tenant provides an individual activation config for each
